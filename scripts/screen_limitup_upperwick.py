#!/usr/bin/env python3
"""選股條件 v1：前一交易日漲停 + 最近交易日上引紅 + 最近一日成交量 >= 1000 張。

預設以 data/twse/2026 與 data/tpex/2026 的本地日資料執行。

條件定義（第一版）：
1. 前一交易日漲停
   - TWSE：以前一日收盤價推算漲停價，比對最近前一交易日 close 是否等於漲停價，且 high == close
   - TPEX：優先使用更前一交易日檔案中的「次日漲停價」比對，若缺值則退回 TWSE 同樣的推算法
2. 最近交易日上引紅
   - close > open
   - high > close（只要有上影線即成立）
3. 最近交易日成交量 >= 1000 張（1 張 = 1000 股）

輸出：
- 終端摘要
- JSON 檔：outputs/screen_limitup_upperwick_<latest_date>.json
"""
...
def resolve_dates(latest_date: str | None, prev_date: str | None) -> tuple[str, str, str]:
    dates = available_dates()
    if len(dates) < 3:
        raise SystemExit("可用交易日不足 3 天，無法判斷前一交易日與其前一日。")
    ...
    if latest_idx < 2:
        raise SystemExit(f"latest_date={latest_date} 前面不足兩個交易日。")

    inferred_prev = dates[latest_idx - 1]
    prev_prev_date = dates[latest_idx - 2]

    if prev_date is None:
        prev_date = inferred_prev
    elif prev_date != inferred_prev:
        if prev_date not in dates:
            raise SystemExit(f"找不到 prev_date={prev_date} 的 TWSE/TPEX 共同資料。")
        prev_idx = dates.index(prev_date)
        if prev_idx < 1:
            raise SystemExit(f"prev_date={prev_date} 前面不足一個交易日。")
        prev_prev_date = dates[prev_idx - 1]

    return latest_date, prev_date, prev_prev_date
...
def same_price(a: float, b: float) -> bool:
    tol = max(tick_size(max(a, b)).__float__() / 2.0, 0.01)
    return abs(a - b) <= tol + EPS


def is_limit_up(prev_bar: DailyBar, prior_bar: DailyBar) -> tuple[bool, float]:
    explicit_limit = prior_bar.next_limit_up if prev_bar.market == "tpex" else None
    limit_price = explicit_limit if explicit_limit is not None else limit_up_price(prior_bar.close)

    close_hit = same_price(prev_bar.close, limit_price)
    high_hit = same_price(prev_bar.high, prev_bar.close)
    return close_hit and high_hit, limit_price


def is_upper_wick_red(bar: DailyBar) -> tuple[bool, float, float, float | None]:
    body = bar.close - bar.open
    upper_shadow = bar.high - bar.close
    ok = (bar.close > bar.open + EPS) and (bar.high > bar.close + EPS)
    ratio = None
    if body > EPS:
        ratio = upper_shadow / body
    return ok, upper_shadow, body, ratio
...
def build_future_days(
    ...
    for future_date in future_dates:
        ...
        pct_from_prev = (
            ((future_bar.close - prev_close) / prev_close) * 100 if abs(prev_close) > EPS else None
        )
        rows.append(
            {
                "date": future_date,
...
def screen(latest_date: str, prev_date: str, prev_prev_date: str) -> list[Candidate]:
    latest_map = load_market(latest_date)
    prev_map = load_market(prev_date)
    prev_prev_map = load_market(prev_prev_date)
    ...
    shared_codes = sorted(set(latest_map) & set(prev_map) & set(prev_prev_map))
    for code in shared_codes:
        latest_bar = latest_map[code]
        prev_bar = prev_map[code]
        prev_prev_bar = prev_prev_map[code]

        hit_limit, limit_price = is_limit_up(prev_bar, prev_prev_bar)
        if not hit_limit:
            continue

        upper_ok, upper_shadow, body, ratio = is_upper_wick_red(latest_bar)
        if not upper_ok:
            continue
        ...
        candidates.append(
            Candidate(
                ...
                prev_limit_up_price=limit_price,
...
def write_output(latest_date: str, prev_date: str, prev_prev_date: str, candidates: Iterable[Candidate]) -> Path:
    ...
    output_path = OUTPUT_DIR / f"screen_limitup_upperwick_{latest_date}.json"
    candidate_list = [asdict(c) for c in candidates]
    payload = {
        "strategy": "limitup_prevday_then_upperwick_red",
        "definition": {
            "prev_day_limit_up": "前一交易日收盤等於漲停價，且當日最高價=收盤價",
            "latest_upper_wick_red": "最近交易日 close > open 且 high > close",
...
def print_summary(latest_date: str, prev_date: str, candidates: list[Candidate], output_path: Path | None) -> None:
    print("策略：前一交易日漲停 + 最近交易日上引紅 + 成交量>=1000張")
    ...
    for item in candidates:
        ratio_text = "-" if item.latest_upper_shadow_ratio is None else f"{item.latest_upper_shadow_ratio:.2f}"
        future_text = ", ".join(
...
def main() -> None:
    parser = argparse.ArgumentParser(description="選出前一交易日漲停、最近交易日上引紅、且成交量>=1000張的股票")
    parser.add_argument("--latest-date", help="最近交易日 YYYYMMDD，預設使用資料中最新日期")
    parser.add_argument("--prev-date", help="前一交易日 YYYYMMDD，預設自動推回前一天")
    parser.add_argument("--no-save", action="store_true", help="只輸出到 stdout，不寫 JSON 檔")
    args = parser.parse_args()

    latest_date, prev_date, prev_prev_date = resolve_dates(args.latest_date, args.prev_date)
    candidates = screen(latest_date, prev_date, prev_prev_date)
    output_path = None if args.no_save else write_output(latest_date, prev_date, prev_prev_date, candidates)
