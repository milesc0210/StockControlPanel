...
- 均線多頭新成形選股
- 標準選股 / 保守選股
- 低基期選股：不以一年低點判斷；使用 60～100 日市場相對排名、官方產業／人工題材族群相對漲幅、近期均線與量能轉強評分
- 今日漲停 / 族群分析
- 60 日 K 線圖檢視
- 40 日 K 線縮圖批次載入
- 本機 SQLite / 記憶體快取
...
- `今日漲停`：綜合成交量與開盤推升至漲停的強度計分，依分數高到低顯示。
- `漲停紅箭`：綜合成交量、紅 K 實體與上影線品質計分，依分數高到低顯示。
- `均線多頭新成形`：綜合量能倍數與 MA5／MA10／MA20 間距計分；畫面仍維持既有族群排序，不改成分數排序。

排序分數只用於同一個選股策略內比較候選股優先度，不代表保證報酬，也不建議跨策略直接比較。
...
分析成功後會依「選股功能＋交易日期」寫入本機 SQLite。關閉並重新啟動程式、切回同一日期時會自動顯示原分析，不會再次呼叫 Hermes。只有該頁已經有 Serenity 分析時按下 `強制重跑`，程式才會先更新選股結果，再以新候選清單強制重新執行 Serenity 並覆蓋快取。
